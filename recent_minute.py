...
def firstrun_candle(klines): return candle_color(firstrun_Open(klines), firstrun_Close(klines), firstrun_High(klines), firstrun_Low(klines))
...
def fourth_candle(klines): return candle_color(fourth_Open(klines), fourth_Close(klines), fourth_High(klines), fourth_Low(klines))
...
def third_candle(klines) : return candle_color(third_Open(klines), third_Close(klines), third_High(klines), third_Low(klines))
...
def second_candle(klines): return candle_color(second_Open(klines), second_Close(klines), second_High(klines), second_Low(klines))
...
def current_candle(klines): return candle_color(current_Open(klines), current_Close(klines), current_High(klines), current_Low(klines))

def candle_color(open, close, high, low):
    if close > open: return "GREEN"
    elif open > close: return "RED"
