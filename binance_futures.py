...
def mark_price()                : return float(client.futures_mark_price(symbol=config.pair, timestamp=get_timestamp()).get('markPrice'))
...
def KLINE_INTERVAL_12HOUR()     : return client.get_klines(symbol=config.pair, limit=query, interval=Client.KLINE_INTERVAL_12HOUR)
