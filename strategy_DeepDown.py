...
def lets_make_some_money(i):
    ...
    if position_info == "LONGING":
        if EXIT_LONG(response, profit_threshold, klines_1min, klines_1HOUR):
            binance_futures_api.close_position(i, "LONG")
        else: print(colored("ACTION           :   HOLDING_LONG", "green"))

    elif position_info == "SHORTING":
        if EXIT_SHORT(response, profit_threshold, klines_1min, klines_1HOUR):
            binance_futures_api.close_position(i, "SHORT")
        else: print(colored("ACTION           :   HOLDING_SHORT", "red"))

    else:
        if GO_LONG(klines_1min, klines_15min, klines_30min, klines_1HOUR) :
            binance_futures_api.open_position(i, "LONG", config.quantity[i])
        elif GO_SHORT(klines_1min, klines_15min, klines_30min, klines_1HOUR):
            binance_futures_api.open_position(i, "SHORT", config.quantity[i])
        else: print("ACTION           :   🐺 WAIT 🐺")

    print("Last action executed @ " + datetime.now().strftime("%H:%M:%S") + "\n")

def GO_LONG(klines_1min, klines_15min, klines_30min, klines_1HOUR):
    if  candlestick.hybrid(klines_1min) == "GREEN" and \
        ...
        candlestick.hybrid(klines_1HOUR) == "GREEN": return True

def GO_SHORT(klines_1min, klines_15min, klines_30min, klines_1HOUR):
    if  candlestick.hybrid(klines_1min) == "RED" and \
        candlestick.hybrid(klines_15min) == "RED" and \
        candlestick.hybrid(klines_30min) == "RED" and \
        candlestick.hybrid(klines_1HOUR) == "RED": return True

def EXIT_LONG(response, profit_threshold, klines_1min, klines_1HOUR):
    if get_position.profit_or_loss(response, profit_threshold) == "PROFIT":
        if candlestick.hybrid(klines_1min) == "RED": return True
    else:
        if candlestick.hybrid(klines_1HOUR) == "RED" or heikin_ashi.VALID_CANDLE(klines_1HOUR) == "RED": return True

def EXIT_SHORT(response, profit_threshold, klines_1min, klines_1HOUR):
    if get_position.profit_or_loss(response, profit_threshold) == "PROFIT":
        if candlestick.hybrid(klines_1min) == "GREEN": return True
    else:
        if candlestick.hybrid(klines_1HOUR) == "GREEN" or heikin_ashi.VALID_CANDLE(klines_1HOUR) == "GREEN": return True
