...
def lets_make_some_money():
    for i in range(len(config.pair)):
        ...
        klines_2HOUR = binance_futures.KLINE_INTERVAL_2HOUR(i)
        ...
        heikin_ashi.output_current(mark_price, klines_5min)
...
def GO_LONG(mark_price, klines_1min, klines_5min, klines_30MIN, klines_1HOUR, klines_6HOUR):
    if not hot_zone(klines_30MIN, klines_6HOUR) and not volume.volume_declining(klines_1HOUR):
        if (current_candle(klines_1HOUR) == "GREEN" or current_candle(klines_1HOUR) == "GREEN_INDECISIVE") and strength_of_current(mark_price, klines_1HOUR) == "STRONG" and \
           (current_candle(klines_5min) == "GREEN" or current_candle(klines_5min) == "GREEN_INDECISIVE") and strength_of_current(mark_price, klines_5min) == "STRONG" and \
           (current_candle(klines_1min) == "GREEN" and strength_of_current(mark_price, klines_1min) == "STRONG") and \
...
def GO_SHORT(mark_price, klines_1min, klines_5min, klines_30MIN, klines_1HOUR, klines_6HOUR):
    if not hot_zone(klines_30MIN, klines_6HOUR) and not volume.volume_declining(klines_1HOUR):
        if (current_candle(klines_1HOUR) == "RED" or current_candle(klines_1HOUR) == "RED_INDECISIVE") and strength_of_current(mark_price, klines_1HOUR) == "STRONG" and \
           (current_candle(klines_5min) == "RED" or current_candle(klines_5min) == "RED_INDECISIVE") and strength_of_current(mark_price, klines_5min) == "STRONG" and \
           (current_candle(klines_1min) == "RED" and strength_of_current(mark_price, klines_1min) == "STRONG")  and \
            war_formation(mark_price, klines_5min) and war_formation(mark_price, klines_1min): return True
...
def EXIT_LONG(response, mark_price, profit, klines_1min, klines_30MIN, klines_1HOUR, klines_6HOUR):
    if get_position.profit_or_loss(response, profit) == "PROFIT":
        ...
        if not hot_zone(klines_30MIN, klines_6HOUR) and clear_direction(mark_price, klines_1HOUR) == "RED" and clear_direction(mark_price, klines_6HOUR) == "RED": return True
...
def EXIT_SHORT(response, mark_price, profit, klines_1min, klines_30MIN, klines_1HOUR, klines_6HOUR):
    if get_position.profit_or_loss(response, profit) == "PROFIT":
        ...
        # Cut loss when both the 1HOUR and 6HOUR is going against you
        if not hot_zone(klines_30MIN, klines_6HOUR) and clear_direction(mark_price, klines_1HOUR) == "GREEN" and clear_direction(mark_price, klines_6HOUR) == "GREEN": return True
...
def THROTTLE_LONG(response, mark_price, klines_1HOUR, klines_2HOUR, klines_6HOUR):
    if clear_direction(mark_price, klines_6HOUR) != "RED" and get_position.get_positionSize(response) < (config.quantity * max_throttle_size) and \
...
def THROTTLE_SHORT(response, mark_price, klines_1HOUR, klines_2HOUR, klines_6HOUR):
    if clear_direction(mark_price, klines_6HOUR) != "GREEN" and get_position.get_positionSize(response) < (config.quantity * max_throttle_size) and \
        get_position.get_unrealizedProfit(response) < get_position.get_margin(response) * throttle_threshold: return True
