...
def DEAD_OR_ALIVE():
    ...
    if position_info == "LONGING":
        if get_position.get_unRealizedProfit() == "PROFIT":
            if EXIT_LONG_TAKE_PROFIT(one_hour, six_hour):            # Take Profit
                print("ACTION           :   💰 CLOSE_LONG 💰")
                if live_trade: binance_futures.close_position("LONG")
            else: print(colored("ACTION           :   HOLDING_LONG", "green"))
        
        elif get_position.get_unRealizedProfit() == "LOSS":
            if THROTTLE_LONG(six_hour):                                  # Throttle - Pushing the initial entry price higher
                print("ACTION           :   🔥 THROTTLE 🔥")
                record_timestamp(binance_futures.KLINE_INTERVAL_2HOUR())
                if live_trade: binance_futures.throttle("LONG")
            elif TAKE_LOSS("LONG", six_hour):                            # Stoploss - Only take a loss when direction change against you
                print("ACTION           :   😭 CLOSE_LONG 😭")
                if live_trade: binance_futures.close_position("LONG")
            else: print(colored("ACTION           :   HOLDING_LONG", "green"))
        ...
        if get_position.get_unRealizedProfit() == "PROFIT":
            if EXIT_SHORT_TAKE_PROFIT(one_hour, six_hour):               # Take Profit
                print("ACTION           :   💰 CLOSE_SHORT 💰")
                if live_trade: binance_futures.close_position("SHORT")
            else: print(colored("ACTION           :   HOLDING_SHORT", "red"))
        
        elif get_position.get_unRealizedProfit() == "LOSS":
            if THROTTLE_SHORT(six_hour):                                 # Throttle - Pulling the initial entry price power
                print("ACTION           :   🔥 THROTTLE 🔥")
                record_timestamp(binance_futures.KLINE_INTERVAL_2HOUR())
                if live_trade: binance_futures.throttle("SHORT")
            elif TAKE_LOSS("SHORT", six_hour):                           # Stoploss - Only take a loss when direction change against you
                print("ACTION           :   😭 CLOSE_SHORT 😭")
                if live_trade: binance_futures.close_position("SHORT")
            else: print(colored("ACTION           :   HOLDING_SHORT", "red"))
...
def JACK_RABBIT():
    ...
    if position_info == "LONGING":
        if  EXIT_LONG_TAKE_PROFIT(one_hour, six_hour) or TAKE_LOSS("LONG", six_hour):
            print("ACTION           :   💰 CLOSE_LONG 💰")
            if live_trade: binance_futures.close_position("LONG")
        else: print(colored("ACTION           :   HOLDING_LONG", "green"))

    elif position_info == "SHORTING":
        if  EXIT_SHORT_TAKE_PROFIT(one_hour, six_hour) or TAKE_LOSS("SHORT", six_hour):
            print("ACTION           :   💰 CLOSE_SHORT 💰")
            if live_trade: binance_futures.close_position("SHORT")
        else: print(colored("ACTION           :   HOLDING_SHORT", "red"))
...
def EXIT_LONG_TAKE_PROFIT(one_hour, six_hour):
    if  (((one_hour_exit_test("LONG") or one_hour == "RED" or one_hour == "RED_INDECISIVE") and volume_confirmation("1HOUR")) or \
        ((six_hour  == "GREEN" or six_hour  == "GREEN_INDECISIVE") and strength_of("6HOUR") == "WEAK")): return True

def EXIT_SHORT_TAKE_PROFIT(one_hour, six_hour):
    if  (((one_hour_exit_test("SHORT") or one_hour == "GREEN" or one_hour == "GREEN_INDECISIVE") and volume_confirmation("1HOUR")) or \
        ((six_hour  == "RED" or six_hour  == "RED_INDECISIVE") and strength_of("6HOUR") == "WEAK")): return True

def TAKE_LOSS(POSITION, six_hour):
    if POSITION == "LONG":
        if  ((six_hour  == "RED" or six_hour  == "RED_INDECISIVE") and strength_of("6HOUR") == "STRONG"): return True

    elif POSITION == "SHORT":
        if  ((six_hour  == "GREEN" or six_hour  == "GREEN_INDECISIVE") and strength_of("6HOUR") == "STRONG"): return True

def THROTTLE_LONG(six_hour):
    if  ((six_hour != "RED") and volume_confirmation("1HOUR")) and (retrieve_timestamp() != current_kline_timestamp(binance_futures.KLINE_INTERVAL_2HOUR())): return True

def THROTTLE_SHORT(six_hour):
    if  ((six_hour != "GREEN") and volume_confirmation("1HOUR")) and (retrieve_timestamp() != current_kline_timestamp(binance_futures.KLINE_INTERVAL_2HOUR())): return True
